...
namespace Module\Musiview\Payment;
...
class Gateway {
	...
	public function __construct(private State $system) {
		...
		$this->stripe = new Stripe\StripeClient($this->system->env['STRIPE_KEY']);
	}
	...
	public function redirectToPaymentItem(string $collectionName, string $collectionFilePath) {
		...
		$product = $this->stripe->products->create(
			...
			['idempotency_key' => 'ik-product-' . sha1($productName . $description . $itemUrl . $this->system->env['STRIPE_KEY'] . '2')]
		);
		...
		$price = $this->stripe->prices->create([
			...
			['idempotency_key' => 'ik-price-' . sha1($price . $currency . $product->id . $this->system->env['STRIPE_KEY'])]
		);
		...
		$paymentLink = $this->stripe->paymentLinks->create([
			...
			['idempotency_key' => 'ik-payment-link-' . sha1($productName . $price->id . $this->system->env['STRIPE_KEY'])]
		);
